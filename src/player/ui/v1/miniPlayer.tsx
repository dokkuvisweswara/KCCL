...
export default function MiniPlayer({ isPlaying, onPlayPause, onNextTrack, onPrevTrack, duration, currentTime, onSeek, volume, onVolumeChange, formattedTime, contentData, isContentLoaded, trackPosition, myPlaylist, actRepeatEnabled, actShuffleWork, myQueuePlaylist, isMute, getVTT, isReplayActive, onReplay, speedOption, onHandleSelectSpeed, activeSpeedTab, speedOptionEnabled }: any) {
  ...
  return (
    <div
      className={`fixed bg-secondaryBgColor divAminations ${flagEnabled ? "queue" : "queue-off"
        }`}
        ref={wrapperRef}
    >
      {exitFullScreen &&
...
{/* 
        <div>
          <p className="border-b border-detailsbordercolor "></p>
        </div> */}
      </div>
      {flagEnabled && (
    
        <div className="divAminations  overflow-auto h-[80vh] mx-2">
