...
export default function BasePlayer({ currentPlaybackPosition, primaryPlayListLocalState, contentData, lang }: any) {
  const router = useRouter();
  const browser = detect();

  const newFlowPlayerAdded = true;
  ...
  useEffect(() => {
    if (trackPosition != null) {
      ...
      let track = myPlaylist[trackPosition];
      ...
      // let analyticsData= playedObject(track,currentTime,audioRef.current.duration)
      // playedEvent(analyticsData)
    }
  }, [trackPosition, myPlaylist]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    if ((information.errorcode == 8882 || information.isAccess == false) && !(information.clearlead && information.clearlead > 0)) {
      ...
      if (availabilityList.length > 0) {
        ...
        if (pricemodel == "RENTAL" || pricemodel == "PAID") {
          ...
          setPriceclassdetailList({priceclassdetail, pricemodel, contentData : primaryPlayListLocalState[currentPlaybackPosition], licenseduration})
  ...
  async function actInitialization(filteredPlayerObj: any) {
    ...
    if (newFlowPlayerAdded) {
      actAccessContent(filteredPlayerObj);
      return;
    }

    const { isPlaybackAllowed, playback_details } = getContentInformation(filteredPlayerObj);
    let drmInfo: any = {};
    let packageInfo: any = null;

    if (isPlaybackAllowed == true && playback_details) {

      packageInfo = await actGetPackageURL(playback_details, getAccessTokenObj());
      if (!packageInfo) {
        notify(errors_message.NO_STREAM, 'error');
      }
      if (playback_details.drmscheme !== 'NONE') {
        drmInfo = await actGetDRMToken(playback_details, getAccessTokenObj())
        if (drmInfo?.errorcode) {
          notify(drmInfo.reason, 'info');
          return;
        }
      }
      ...
      if (!sessionToken) {
        router.push('/')
      } else if (!playback_details) {
        router.push('/subscribe');
      } else {
        notify(errors_message.AVAILABILITY, 'error');
      }
    }
  ...
  const handleNextTrack = () => {
    ...
    if (myPlaylist.length - 1 != trackPosition) {
      setTrackPosition((prevTrack: number) => (prevTrack + 1));
    } else {
      if (repeatType == 'rept-all') {
        setTrackPosition(0)
      } else {
